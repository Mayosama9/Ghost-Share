document.addEventListener('DOMContentLoaded', () => {
    ...
    function addItem(name, type, size = 0,num, modified = new Date().toLocaleString()) {
    ...
    function renderItems() {
        ...
        currentDir.items.forEach(item => {
            ...
            shareBtn.onclick = function() {
                ...
                share_box.innerHTML=`<p style=" padding-bottom:10px ;">Copy Link:</p> <span class="close">&times;</span><a href="http://127.0.0.1:8000/download/${item.num}" target="_blank">http://127.0.0.1:8000/download/${item.num}</a>`;
            }
    ...
    async function fetchUserData() {
        try {
            ...
            console.log(data);
            data.forEach(item => {
                addItem(item.filename, item.file_type, item.file_size,item.num, item.modified);
            });
        } catch (error) {
